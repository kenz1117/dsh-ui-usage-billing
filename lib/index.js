	...
	const aggregateOnce = async () => {
		...
		for (const meta of metas) {
			...
			try {
				...
				if (ledgerRow !== void 0 && ledgerRow.stamp !== void 0 && stamp !== null && ledgerRow.stamp === stamp && (ledgerRow.foldVersion ?? 1) === 12 && ledgerRow.fingerprint === foldFingerprint) {
		...
		if (options.ledger !== void 0) {
			for (const entry of ledger.values()) {
				...
				try {
					const stale = (entry.foldVersion ?? 1) < 12 || entry.fingerprint !== foldFingerprint;
					folds.push({
						id: entry.id,
