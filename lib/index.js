...
function queryXai(ctx, apiKeyEnv) {
	return queryBearerBalance(ctx, XAI_CREDITS_URL, apiKeyEnv, "xAI", "xAI", (data) => {
		const cents = toNumber(data.total?.val);
...
async function queryRelayQuota(ctx, route) {
	...
	if (cached !== void 0 && Date.now() - cached.at < FINGERPRINT_TTL_MS) return {
		...base,
		kind: cached.kind,
		status: "ok"
	};
...
function isLoopbackHost(req) {
	...
	return name === "localhost" || name === "::1" || name !== void 0 && name.startsWith("127.");
}
/**
...
function apply(ctx, config = {}) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			for (const row of [...balances, ...custom]) if (row.error === "unauthorized") warnAuthOnce("balance", row.provider, row.displayName);
