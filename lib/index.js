...
function applyLivePricing(pricing) {
	...
	typeof pricing.rateEur === "number" && Number.isFinite(pricing.rateEur) && pricing.rateEur > 0 && pricing.rateEur;
...
*/
function tierAtWithBounds(timeMs, bounds, holidays = false) {
...
*/
const CHINA_HOLIDAYS = "20260925 20260926 20260927 20261001 20261002 20261003 20261004 20261005 20261006 20261007";
/** 北京日期键 YYYYMMDD（与宿主时区无关）。 */
function beijingDateKey(timeMs) {
	return new Date(timeMs + 288e5).toISOString().slice(0, 10).replace(/-/g, "");
}
/** 时刻是否落在北京时间法定节假日（仅覆盖 {@link CHINA_HOLIDAYS} 内已收录的年份）。 */
function isBeijingHoliday(timeMs) {
...
function createUsageAggregator(persistence, options = {}) {
	...
	*  LEDGER_SAVE_INTERVAL_MS 节流。失败仅告警——内存中的聚合值仍然正确。 */
	const saveLedger = async (force) => {
		if (options.ledger === void 0 || !ledgerNeedsSave) return;
		if (!force && lastLedgerSaveAt !== 0 && Date.now() - lastLedgerSaveAt < 6e4) return;
		try {
	...
	const aggregateOnce = async () => {
		...
		for (const key of [...unreadable.keys()]) if (!seen.has(key)) unreadable.delete(key);
	...
	return {
		aggregate() {
			if (lastDoc !== void 0 && Date.now() - lastAt < 5e3) return Promise.resolve(lastDoc);
			inflight ??= aggregateOnce().finally(() => {
				inflight = void 0;
			});
			return inflight;
		},
...
*/
async function fetchRateFrom(sources) {
...
const FIRST_PARTY_PRICE_SOURCES = /* @__PURE__ */ new Set([
	"anthropic",
	"openai",
	"google",
	"mistral",
...
function buildExtraModels(data) {
	...
	const catalogKeys = /* @__PURE__ */ new Set([...BUILTIN_MODEL_CATALOG.map((entry) => entry.key.toLowerCase()), ...Object.keys(BUILTIN_MODEL_KEY_ALIASES).map((key) => BUILTIN_MODEL_KEY_ALIASES[key]?.toLowerCase() ?? key.toLowerCase())]);
...
async function fetchLivePricing() {
	const [rate, rateEur, models, modelsDev] = await Promise.all([
		fetchRate(),
		fetchRateFrom(EUR_RATE_SOURCES),
		fetchRouterModels(),
	...
	if (rate === void 0 && rateEur === void 0 && prices === void 0 && (extraModels === void 0 || extraModels.length === 0)) return { source: "builtin" };
	return {
		source: "live",
		...rate !== void 0 ? { rate } : {},
		...rateEur !== void 0 ? { rateEur } : {},
...
* 回退（订阅查询走「未读出」兜底而非抛错）。
*/
function subscriptionOrigin(configBaseUrl, fallback) {
	if (configBaseUrl === void 0 || configBaseUrl.trim() === "") return fallback;
	try {
...
function resolveMiniMaxBaseUrl(config) {
	const fallback = config.provider === "minimax-cn" || config.provider === "minimax-token-plan-cn" ? "https://api.minimaxi.com" : "https://www.minimaxi.com";
...
function apply(ctx, config = {}) {
	...
	const ledgerStore = createFileUsageLedgerStore(config.ledgerPath ?? join(dshHome, ".dsh-usage-ledger.json"));
	...
	ctx.inject(["settings"], (sctx) => {
		const legacy = sctx.settings;
		if (typeof legacy.register !== "function") return;
		const scope = legacy.register(usageBillingSettingsNs, UsageBillingSettingsSchema, { base: { enableUsageStatsTool: config.enableUsageStatsTool ?? false } });
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			aggregating.then((stats) => {
				persistSnapshot(decorate(stats));
			}, () => {});
			let fresh;
			try {
				fresh = await Promise.race([aggregating.then((stats) => stats), new Promise((resolve) => {
					setTimeout(() => {
						resolve(void 0);
					}, RESPONSE_BUDGET_MS).unref();
				})]);
			} catch (error) {
