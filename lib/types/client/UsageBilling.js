...
/**
 * Apply the node half's live pricing snapshot (rate + router price overrides).
 * Any failure keeps the built-in catalog and rate — degrade, never fabricate.
 */
...
export function UsageBilling(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(UsageBillingTrigger, { ...props, onOpen: openDashboard, monthCost: monthCost, todayCost: stats.byDay[today]?.cost ?? 0 }), open && (_jsx(BillingDashboard, { stats: stats, t: t, onClose: close, health: health }))] }));
}
