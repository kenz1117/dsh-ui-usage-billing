 ...
 */
export function TrendChart({ data, models = [], currency = 'cny' }) {
    ...
    const layout = useMemo(() => {
        ...
        const maxCost = Math.max(...data.map(d => Math.max(d.cost, Object.values(d.byModel ?? {}).reduce((sum, v) => sum + v, 0))), 0.0001);
    ...
    return (_jsxs("div", { className: css.chartWrap, children: [_jsxs("svg", { viewBox: `0 0 ${W} ${H}`, className: css.chartSvg, role: "img", "aria-label": "Daily cost by model and total calls", onMouseLeave: () => { setHover(null); }, onMouseMove: (e) => {
                    ...
                    setHover(Math.min(Math.max(index, 0), n - 1));
                        ...
                        const y = yCost(value);
                        return (_jsxs("g", { children: [_jsx("line", { x1: PAD.left, x2: W - PAD.right, y1: y, y2: y, className: css.chartGrid }), _jsx("text", { x: PAD.left - 8, y: y + 3, textAnchor: "end", className: css.chartAxisLabel, children: money(value) })] }, `cost-${idx}`));
                    }), bars.map(bar => (bar.value > 0 ? (_jsx("rect", { x: bar.x, y: yCost(bar.base + bar.value), width: barW, height: yCost(bar.base) - yCost(bar.base + bar.value), rx: bar.topRounded ? 2 : 0, className: bar.model.color === '' ? css.chartBar : css.chartStack, style: bar.model.color === '' ? undefined : { fill: bar.model.color } }, `${bar.date}-${bar.model.key}`)) : null)), _jsx("path", { d: linePath, fill: "none", className: css.chartLine }), callsTicks.map((value, idx) => {
