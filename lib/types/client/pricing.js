 ...
 */
export function isPromoActive(promo, nowMs) {
 ...
 * @param nowMs - 判定时刻（epoch ms）。
 */
export function applyPromo(entry, nowMs) {
    const { promo } = entry;
    if (promo === undefined || !isPromoActive(promo, nowMs))
        return entry;
    const scaled = (band) => ({
 ...
 */
export function catalogEntries(nowMs = Date.now()) {
    const entries = [...MODEL_CATALOG.map(entry => applyPromo(entry, nowMs)), ...(liveExtraModels ?? []).map(extraEntryOf)];
 ...
 */
export function computeCost(entry, buckets, peakShare = DEFAULT_PEAK_SHARE, nowMs = Date.now()) {
...
export function computeCostAt(entry, buckets, timeMs, peakShare = DEFAULT_PEAK_SHARE) {
    ...
    if (priced.price.offPeak === undefined)
        return priceBandCost(priced.price, buckets, priced.price.currency);
