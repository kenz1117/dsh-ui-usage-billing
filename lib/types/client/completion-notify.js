...
const DEFAULT_CONFIG = { enabled: false, timeout: 0 };
/** 读取本地配置（缺失/损坏时回退默认）。 */
 ...
 */
export function installCompletionNotifier(list, getConfig) {
    // 上次快照里每个会话是否已「完成」（用 Set 记录已完成的会话 id）。
    const previousFinished = new Set();
    // 首见会话集合：第一次进入视野的会话 id（无论完成与否），只建基线、不提醒。
    // 用它把「新出现的已完成会话（宿主恢复历史会话/迟到快照）」与「running→completed 迁移」区分开。
    const seen = new Set();
    // 当前自动关闭通知的定时器（重入时先清旧，避免并发通知互顶）。
    let closeTimer;
    ...
    for (const id of snapshot.ids) {
        ...
        if (summary !== undefined) {
            ...
            if (isFinished(summary))
                previousFinished.add(String(id));
        }
    }
    const notify = (title) => {
        ...
        try {
            ...
            // 非 0 = 秒后自动关闭（契约承诺）；0 = 常驻（requireInteraction），不主动关。
            if (timeout > 0) {
                if (closeTimer !== undefined)
                    clearTimeout(closeTimer);
                closeTimer = setTimeout(() => notification.close(), timeout * 1000);
            }
        }
        catch {
    ...
    const unsubscribe = list.subscribe(() => {
        ...
        for (const id of state.ids) {
            ...
            if (firstSeen)
                seen.add(key);
            // 只有「之前已见且运行中 → 现在已完成」才提醒；首见即已完成的会话只入基线。
            if (finished && !firstSeen && !previousFinished.has(key)) {
                finishedTitles.push(summary.title ?? summary.id ?? key);
            }
            if (finished)
                previousFinished.add(key);
            else
                previousFinished.delete(key);
        }
...
//# sourceMappingURL=completion-notify.js.map