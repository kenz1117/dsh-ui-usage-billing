...
import { queryDeclaredEndpoints } from "./declarative.js";
import { reconcileBalanceDelta } from "./reconcile.js";
...
import { isOfficialBaseUrl, queryRelayQuotas } from "./relay.js";
import { planTypeOf, subscriptionFeeCnyOf } from "./client/plan-knowledge.js";
import { BILLING_SETTINGS_NAMESPACE, DEFAULT_ENABLE_USAGE_STATS_TOOL, ENABLE_USAGE_STATS_TOOL_FIELD, } from "./client/usage-billing-settings.js";
...
const UsageBillingSettingsSchema = z.object({
 ...
 */
export function createFileUsageLedgerStore(ledgerPath) {
    return {
        async load() {
 ...
 */
export function readPiAiProviderRoutes(settings) {
    try {
        const descriptors = settings.describe({ redactSecrets: true });
        const pi = descriptors.find(descriptor => descriptor.ns === 'llm-pi-ai')?.value;
        const providers = pi?.providers;
    ...
    return (cwd) => {
        ...
        try {
            ...
            for (const record of records) {
                if (record.path !== '' && cwd.startsWith(`${record.path}/`))
                    return record.title;
            }
            return undefined;
        }
...
export function apply(ctx, config = {}) {
    ...
    void (async () => {
        try {
            const doc = JSON.parse(await readFile(reconcilePath, 'utf8'));
            const ref = doc.ref;
    ...
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (req, res) => {
            if (!guardLoopback(req, res))
                return;
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' });
            res.end(JSON.stringify(live));
        },
    }), 'usage-billing: pricing route');
    let balanceCache = { at: 0, doc: { balances: [] } };
    ctx.effect(() => ctx.webServer.register({
        kind: 'exact',
            ...
            try {
                ...
                for (const row of [...balances, ...custom, ...declared]) {
                    if (row.error === 'unauthorized')
                        warnAuthOnce('balance', row.provider, row.displayName);
                }
                ...
                if (official !== undefined && official.totalBalance !== undefined) {
                    ...
                    if (result.event !== null && result.event.kind !== 'flat')
                        reconcile = { ...result.event, provider: official.displayName };
                    if (result.ref !== null)
                        persistReconcileRef();
                }
    ...
    const refreshQuotas = async () => {
        ...
        for (const row of rows) {
            if (row.status === 'unauthorized')
                warnAuthOnce('subscription', row.provider, row.displayName);
        }
