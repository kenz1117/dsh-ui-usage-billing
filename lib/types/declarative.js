 ...
 */
export function normalizeDeclaredOrigin(baseURL) {
 ...
 */
export function readDeclaredPath(body, path) {
...
function windowUsedPercent(body, input) {
    ...
    if (usageRatio !== undefined)
        return clampPercent(usageRatio * 100);
 ...
 */
export async function queryDeclaredEndpoints(ctx, providers, declarations) {
    ...
    return await Promise.all(declarations.map(async (decl) => {
        ...
        try {
            ...
            return {
                ...
                ...(nothingResolved ? { error: 'invalid', reason: '声明路径未命中任何字段' } : {}),
            };
        }
        catch {
...
async function fetchDeclaredBody(url, raw, apiKey) {
    const doRequest = async () => {
        ...
        try {
            ...
            if (!response.ok)
                throw new HttpStatusError('http-error', response.status);
            const text = await response.text();
            // 规则 6：响应体上限。
            if (text.length > MAX_BODY_BYTES)
                throw new HttpStatusError('body-too-large', 413);
