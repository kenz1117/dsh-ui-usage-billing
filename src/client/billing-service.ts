...
export function createBillingMetrics(): BillingMetricsRuntime {
  let costs: BillingCosts | undefined
  let open: (() => void) | undefined
  const listeners = new Set<(costs: BillingCosts | undefined) => void>()
  return {
    readCosts: () => costs,
    ...
    registerOpen: (handler) => {
      open = handler
      return () => { if (open === handler) open = undefined }
    },
  }
}
