...
- **每日 Token 堆叠**：双视角切换（7/30 天切换共用）——「按结构」输入(缓存未命中)/输入(缓存命中)/输出
  三桶分色柱状；「按模型」每天按模型堆叠（段 = 模型总 token，分色复用趋势页 `chartModels`，
  图例全量展示按窗口总量降序；旧快照缺 `byDayModels` 时隐藏切换钮）。
- **聚焦**：点击图例色块或模型 Token 表行聚焦单个模型——目标段保持原色、其余段弱化至 15%，
  y 轴与柱总高不变；再点解除；7/30 天切换时聚焦模型仍在窗口则保持，切回按结构即清除。
- **悬停明细**：结构视角给当日三桶精确值；模型视角给逐模型「命中/未命中/输出」行（无 reasoning 列，
  `byDayModels` 无该字段）。
- **模型 Token 占比**：模型排行 + 占比分段条（行可点，与聚焦联动）。
- **Token 结构 KPI**：缓存命中率 / 思考占比 / 输入输出比 / 峰值日（恒为全局结构口径，不随聚焦变化）。
- **导出**：按日 Token CSV（结构口径）+ JSON（含按日×模型明细，无 reasoning 列）。
- **性能面板**：按模型 TTFT 均值/P50/P90、生成速度、总延迟 + 按小时×模型对比曲线；
  「首字延时 (ms) / 生成速度 (tok/s)」指标 tab 切换单轴视图，模型 chip 可点击开/关曲线
  （默认点亮样本数前 5，「全选」一键点亮；tab 与点亮集合 localStorage 持久化），
  hover 吸附最近小时显示十字线 + 逐模型数值 tooltip；缺失样本小时断线不 fabricated；
  「估算样本」标注工具续写步骤。
